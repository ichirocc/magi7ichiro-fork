...
| できた・要対応（今すぐ直せる人手不足あり） | **「○月○日が人手不足です」**（黄） | **なおすのを手伝って** | — |
| できた・要対応（必須を減らす1手あり） | **「必須違反が N件 残っています。直す手があります」**（黄） | **直す1手を見る** | — |
| できた・要対応（1手を探している間） | **「…直し方を探しています…」**（黄） | （出さない） | — |
| できた・下限の見込み（1手なし・長く改善しない必須・希望が関わる） | **「今の希望とルールの組み合わせでは、必須違反 N件 が下限の見込みです」**（黄） | **ぶつかっている希望を見る** | このまま書き出す |
| できた・要対応（関わる希望あり＝必須に関わる希望か、人手不足の日に別の勤務の希望） | **「…希望とルールがぶつかっています」**（黄） | **ぶつかっている希望を見る** | — |
| できた・要対応（それ以外） | **「必須違反が N件 残っています」**（黄） | **問題を見る** | — |
| 直せない問題が残る（人手不足の日に別の勤務の希望がある） | **「いまの希望のままでは、ここは埋められません」**（赤） | **ぶつかっている希望を見る** | データを見直す |
| 直せない問題が残る | **「このデータでは、ここは埋められません」**（赤・理由つき） | **データを見直す** | そのまま配る |

- **常に主ボタンは1つだけ**を大きく。迷いを作らない。
- 要対応の行は上から順に判定する（3.612.0 思考誘導 S0/S4＝「足りる？→1手ある？→希望が関わる？」）。「直す手」は必須を減らす手だけ。
  「ぶつかっている希望を見る」は関わる希望を名前・日付・理由つきで並べる（押すとそのセルを開く、S3）。
  各行の「取り消したら？」で 1 行ずつ試算し、結果が出た行は「希望を取り消して、もう一度つくる」（赤字・元に戻す 1 回で希望と勤務表が戻る）で確定できる（S5、`docs/s5_wish_trial.md`）。
