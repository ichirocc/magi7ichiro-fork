...
- `MagiAccent`＝シフト/違反アクセント（blue/green/orange/purple/pink/red/gray）。**シフト色はユーザーの認識に
  結びつくため憲法改定でも据え置き**（機能色。テーマ chrome とは別系統）。
...
- **P4 任意角丸**：`RoundedCornerShape(<dp>)` の新規任意値禁止（`MaterialTheme.shapes.*` を使う）。
