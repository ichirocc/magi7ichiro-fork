...
| `weekly` | 2 | SOFT | 7日周期のシフト平準化：職員×シフト×曜日で round(そのシフトの回数/7) からの L1 偏差和（1→2、3.522.0） | 職員×シフト（`distLocations["weekly"]`） |
...
- **7日周期のシフト平準化（weekly）**：職員ごと**シフトごと**に、そのシフトが入る日の**曜日別カウント**の round(そのシフトの回数/7) からの L1 偏差和。`weekday(j)=(dow0+j)%7`（`Problem.dow0`）。「夜勤が毎週おなじ曜日」「休みが毎週おなじ曜日」を同じ式で均す。fair と同型・重み2。
