...
- **適切回数（apt）**：群目標を個人 `staffRange[lo,hi]` でクランプし、固定職員の解消不能な幻 apt を除去。**3.508.0** さらに「構造的に到達できる範囲」へ収める＝到達下限 `T − Σ(他の置けるシフトの実効上限)`・到達上限 `T − Σ(他シフトの実効下限)`（実効下限 = max(lo, 希望固定数)、実効上限 = 置けるなら hi か T、置けないなら希望固定数）。個人 [lo,hi] と矛盾するときは個人設定を優先。評価・最適化は実効目標 `Problem.apt`、設定ミス診断（6b 担当構成の下限・6d 希望固定・6-C 目標合計）は設定値 `Problem.aptRaw` で判定し「計算では N 回を目標として扱っています」と案内する。例: 休 10〜10・有 1〜1・他が上限 0 の職員の B4 は群目標 1 → 20。
