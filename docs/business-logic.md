...
> **最終更新**：2026-09-25（希望どうしの衝突 `wishSelfConflicts`＝設定ミス診断 1c・HF70・残存分析。評価・重みは不変。重みの最終改定は 3.556.0）
...
- **希望どうしの衝突（`V6SanityPort.wishSelfConflicts`）**：希望は固定なので、次の組は希望を 1 件取り消すまで必ず必須違反が 1 件残る（並びが成立すれば c3n/c3w、崩せば pref）。
  ...
  HF70 の「希望以外HARD」（①②の c3n/c3w を差し引き「希望どうしの衝突 N 件」を別に出す）、残存分析（「もう直せない」へ族別に移す）が同じ組を読む。
