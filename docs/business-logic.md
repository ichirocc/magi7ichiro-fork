...
- **希望どうしの衝突（`V6SanityPort.wishSelfConflicts`）**：希望は固定なので、次の組は希望を 1 件取り消すまで必ず必須違反が 1 件残る（並びが成立すれば c3n/c3w、崩せば pref）。
  ...
  **規則 A**（`PolishGate.wishPinStrict`、既定 ON）: 希望固定セルは「希望へ」か「今のまま」だけ＝希望どおりのセルは別の値へ変えない・未反映のセル（割当を手で変えた・希望をあとから足した等）は希望へ戻してよい・希望でも今の値でもない値へは変えない（`Problem.wishMoveAllowed`、盤面ごと採る経路は `Problem.keepsWishPins`）。
  衝突は利用者が希望を取り消す（S5「希望を取り消して、もう一度つくる」）まで c3n/c3w として残り、どの希望を外すかを最適化器が選ぶことはない。未反映のセルは入口 hf67 の採否（改善時のみ）に従いそのまま持ち越すことがあり、手で変えた値が残ることは保証しない（明示の手動固定は backlog #41）。
