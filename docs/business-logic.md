...
> **最終更新**：2026-09-13（3.538.0＝fairの平均計算を達成率モード化、ユーザー明示指示）
...
| `fair` | 2 | SOFT | グループ内公平化：群×担当ONシフトで`Problem.fairDevOfBucket`のL1偏差和（1→2、3.522.0。3.538.0で達成率モード追加、下記参照） | 職員×シフト（`distLocations["fair"]`） |
...
- **グループ内公平化（fair）**：群×担当ONシフトで`Problem.fairDevOfBucket`（`MirrorCore.kt`、Evaluator/DeltaEvaluator/
  ...
  **達成率モード（3.538.0、ユーザー指示「公平化の平均は、目標値と個人上下限などの設定を配慮した達成率スコアに
  変換してから計算する」）**：群の全メンバーに「基準」（`Problem.fairBasisAnchor/Width`＝範囲`staffRange.lo/hi`
  両方が有限なら優先、無ければ実効apt目標`Problem.apt`>=0）が揃う場合だけ発動。各人の達成率=(回数-基準値)/基準幅、
  目標達成率(tgt_rate)は達成率の単純平均（丸めない）。各人の偏差は`|達成率-tgt_rate|×自分の基準幅`を四捨五入し
  件数相当へ戻す（重み2の「件数」という単位は変えない）。基準幅ゼロ（範囲lo==hi・apt目標0）の人は個別扱い：
  回数が基準値と一致すれば偏差0、それ以外は差をそのまま件数化し、tgt_rateの平均対象からは除外する（自分の基準
  ちょうどを100%達成とみなすだけで群平均とは比較しない）。1人でも基準が無い、または達成率の平均対象（基準幅>0の
  人）が1人もいなければ、その群×シフト全体は**従来の生回数`round(平均)`のL1偏差**（3.522.0以前の唯一の式）へ
  フォールバックする。C++は`fairBasisOf`でrangeLo/rangeHi/aptから同じ式をJNI経由の追加データ無しに導出する
  （`fairMarginalAt`/`fairMarginalN`の旧`grpTotal`（群合計）引数は達成率モードで不要になり3.538.0で撤去）。
