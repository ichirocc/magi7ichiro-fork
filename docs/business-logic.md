...
> **最終更新**：2026-09-10（high 重み 45→25、HF77明示指示）
...
> 階層（low90 > c3mn30=c1 30 > high25 > … > 軽い族1）に反して「重い違反を軽い違反の件数と交換する」採用を許していた。
...
| `high` | 25 | SOFT | 個人上限超過（`staffRange.hi`、LimMax。45→25、2026-09-10 HF77明示指示） | 回数 `i,k` |
...
> 2026-08-27（HF77 明示指示）に 1.0→5.0 へ再変更。人員過剰(covO)が個人上限(high, 当時の重み45)に阻まれて
> 研磨されない実機ログを受け、apt/fair/weekly/c2/c41/c42/c41s/c42s（重み1）より確実に優先して削られる
> 水準へ引き上げた。high/low/c1/c3mn（当時45/90/30/30）には遠く及ばない＝個人の労働条件や構造ルールより
> 日々の過剰人員削減を優先しない、という位置づけは維持（4面同時変更＝`MirrorKeys.weights`・
> `Evaluator.fullEvalParts`・`DeltaEvaluator`・`magi_native.cpp`＋言語跨ぎ期待値3ファイル）。
> 2026-09-10（HF77 明示指示）に high を 45→25 へ変更（UI重み表の並びで人員過剰(covO=5)と期間の制約/
> 回避の並び(c1/c3mn=30)の間に来るようにするため）。covO(5) との差は縮まったが依然 high(25) が上＝
> 位置づけ（covOが個人上限より優先されない）は変わらない。
...
- **個人下限/上限（low/high）**：`staffRange["i,k"]={lo,hi}`。`low` は `lo!=0 && 担当可 && 回数<lo`、`high` は `回数>hi`。重み 90/25（被覆や c1 に負けない最低限の重み）。
