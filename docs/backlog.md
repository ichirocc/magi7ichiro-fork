...
> 完了した項目は打消し線のまま残す（同型の課題を二度登録しないため）。開いている項目はこのファイルが正（CLAUDE.md はここを指すだけ）。

1. ~~TallyCard の読取/編集モード完全整合（result専用検査結果の plumbing）~~ **→ 3.96.0 で完了**（ユーザー向け機能の TallyCard 項参照）。
2. 未レビュー領域の精読（**2026-09-25 で全領域完了**）: ~~`V6LateOperators`~~（3.507.7）/~~`V6SearchOperators`~~（3.507.9）/~~`V6HotfixPasses` 各パス内部~~（3.508.1）, ~~CSV~~（3.507.8）/~~UI 層~~（3.508.2 でエンジン契約との接点を確認、**2026-09-25 に全面精読**＝5 群で精読し 52 件＋取りこぼし 5 件を是正、`android-sdk.yml` で実コンパイル。history「画面層の精読 #2 の残り」。C# への移植は同節の一覧）。**（3.508.2）** `V6PortAnalyzer`（3.503.0）・`V6SwapSuggester`（3.507.4）・`V6SanityPort`（3.507.5）・`V6LateOperators`（3.507.7）は一巡。
...
25. ~~**[運用・要個別判断] Dependabotのminor/patch group化設定後、既存の個別メジャー更新PRが残っている**~~ **→ 解消（2026-09-26、#216・#217 を利用者判断でマージ、両 repo ともオープン PR 0）**
...
37. **[S5 任意の残作業・2026-09-24 登録]**（本体は Android 0b1d7b3/96a147f/044268d・C# a56506c..ddbb16d で実装済み、`docs/s5_wish_trial.md`）:
    ...
    ⑥Rk<H0・att≤0 の行も対象にするか。仕様 `s5_wish_trial.md` の「実機で 0 の行が多いと分かってから足す」に従い、実機の 0 の行の頻度を見てから着手を推奨。**利用者の決定（2026-09-25）: 実機で 0 の行が多いと分かってから作る**（それまで未着手）。
38. ~~**[外部机上テスト D2・2026-09-25 登録]** スキルグループ未指定の職員が先頭のスキルグループ（index 0）に入る。~~ **→ 2026-09-25 Android で完了**
    （既定 −1＋最初のスキル群を作るとき全員 −1＝`Ws1Ops.addSkillGroup`、画面の配線は 60e79d4（#221）。C# は `-MAGI_PC` 3e9a5c2 で同日同期。history「backlog #38」）。
...
39. **[外部机上テスト SI-3・2026-09-25 登録、測定済み・否決（2026-09-26）]** 初期解の C1 充足がシフト index 順に依存する。
    ...
    **否決（2026-09-26）**: 不足度順（`c1ScarcityOrder`）を LoopBench 160 組で測定＝勝ち越し 79/31 だが m03 で必須件数増 2 試行（統計ゲート不合格）、実データ 4 件は初期解が同一で効果 0。入れない（`docs/history/3.4xx.md`）。
40. **[希望固定セルの漏れ・2026-09-25 登録、規則 A で修正・測定合格（2026-09-26）]** 利用者の方針（2026-09-25）: 希望どうしが衝突しても最適化器が希望を崩すことは認めない
    （取り消しは S5 で利用者が選ぶ）。ポートフォリオ経路（予算 211 秒以上）で `EliteRelinking.elitePathRelink`・`EliteIntegrationPolish` の端点採用・
    `personSwapKick` が希望を破った盤面を残していた（c3n 9000→pref 8000 は必須件数が同じで重み付きが下がる＝keep-best が採る。実データの古泉 10/25）。
    **利用者の決定（2026-09-25）＝規則 A**: 希望固定セルは「希望へ」か「今のまま」だけ（希望どおりのセルは動かさない・未反映は希望へ戻すのは可・
    希望でも今の値でもない値へは動かさない）。`PolishGate.wishPinStrict`（既定 ON）、判定は `Problem.wishMoveAllowed`/`keepsWishPins`。
    先に決めた「値によらず凍結」はあとから足した希望が載らなくなる（golden 63〜85%）ため不採用。経路の分類と測定は `docs/history/3.4xx.md`
    「希望固定セルの規則 A」。旧 P1（終了時に崩れた希望 0）は未達＝入力時から崩れていた古泉 10/25 が残る（手で決めたかは記録しないので区別できない）。
41. **[明示の手動固定（案 C）・2026-09-25 登録、実装済み（2026-09-26、利用者 go・決定 Q1〜Q5）]** 規則 A は手で変えたセルが残ることを保証しない（未反映の希望固定セルは最適化器が希望へ戻しうる）。
    セル編集シートの［固定する］でそのセルをいまの値のまま最適化器に書き換えさせない（`manualPins`、手動固定＞規則 A の希望へ戻す＞通常の書き換え）。
    手の編集は可で値が追従、採点・重みは不変。Kotlin・C++（`wishLockedN`/`lockToN`）・C# 同日。経緯と受け入れ 7 項目は `docs/history/3.4xx.md`「手動固定」。
