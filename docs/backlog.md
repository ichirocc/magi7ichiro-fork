...
15. **「制約クレジット台帳付き複合修復」（2026-09-08 ユーザー提示設計）＝机上評価済み（history 3.510.2 補遺）、決定待ち**:
    ...
    ビームトリムが単一プールのため負債候補が飢餓しうる構造を発見し `Params.debtLaneSlots`（二車線ビーム、既定 0）を実装＝**未計測**
    （`MAGI_BENCH_FEATURE=debtlane`）。仕様書自体は結合度認識型負債テンソル（§7.1）・Restless Bandit（§12）・並列トランザクション（§13）
    も提案するが、§20「解除条件を満たすまで実装禁止」のため見送り（重い機構をいきなり実装せず、まず1変数の構造修正で iter11 を再計測する）。
