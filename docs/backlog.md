...
24. **[要product判断・実データで実害確認済み＝深刻] `restShiftIndex`（`MirrorCore.kt`）の記号依存＋無言の0退避**
    ...
    **実データ4件（sept2026/blocked_covu/golden/sample_v6、いずれも実運用データの匿名化版）で
    ...
    schema変更**＝業務判断が要る（grilling対象）ため保留するが、実害の深刻さを踏まえると
    **暫定的な軽量対策**（例: `removeShift`が「休」解決先シフトを削除する際に警告する／確認を求める、
    あるいは3.416.0の削除許可自体を再検討する）は優先度を上げて検討する価値がある＝次にgrillingで
    詰めるべき論点として明示的に提示する。
