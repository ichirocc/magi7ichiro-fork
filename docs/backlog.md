...
36. **[完了・3.610.0で既定ON（構造床条件つき）] `postChainRunningKeepBest`（3.608.0）のtools/loop A/B判定**（2026-09-21）。
    ...
    **訂正**: 上の「許容ONのみ 98/115（p=0.27）、許容ON+keep-best 97/86（p=0.46）＝許容ON自体も既定より有意に良いとは
    ...
    腕として測る必要がある）。**→ 再訂正（2026-09-23）**: 同一ベンチ内の正式A/Bは 3.537.0 に既にあった（`iter_aptfairtol.csv`、138ペアで OFF 68勝/ON 38勝・p=0.0046・必須増3＝ON 不利、history 3.537.0）。「未測定」は見落とし。ただし 3.541.0 以降（fair v2・重み改定・走行 keep-best 既定 ON）の前の測定なので、現行コードで `aptfairtol` 腕を再測定する。同一ベンチ内の新旧ペア（A・B・iter8・上の 112/72）は初期解が共通なので有効。
