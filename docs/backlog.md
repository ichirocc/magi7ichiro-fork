...
12. **外部レビュー（2026-09-07、ユーザー提示 3 表）の提案＝決定・測定待ち**（3.509.2 で仕分け。詳細は history 3.509.2）。
    ...
    (b) 探索動学: C2 専用研磨＝3.511.3 `C2Polish` 実装（iter15 で採否）／C41・C42 の最小費用フロー研磨／族選択を件数×重み×改善可能性へ／採用 0 時の探索半径拡張＝3.511.1 `StallEscalationConfig` 実装・iter13 で**不合格**（170ペア全件無変化＝発火条件「クラスタ全体で1巡も採用0」が実質発火しない、既定OFF維持）。発火条件を「直近ラウンドの採用数が閾値未満」等へ緩めた再設計が要る（測定）／C1 重複窓の連結成分化／C3n の前後余白込み LNS／
