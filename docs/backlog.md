...
40. **[希望固定セルの漏れ・2026-09-25 登録、修正と測定中]** 利用者の方針（2026-09-25）: 希望どうしが衝突しても最適化器が希望を崩すことは認めない
    ...
    実装（スクラッチ・測定中）: `PolishGate.wishPinStrict`（既定 true）で relink・personSwapKick・エリート統合の採用 4 か所（端点・relink 途中解・fusion・最終）を塞ぐ。
    **利用者の決定（2026-09-25）: 希望と違う勤務がすでに入っている希望固定セル（手で「割当を変更」した「希望 休（未反映）」、または過去の漏れの残り）は
    そのまま残し、最適化はどの値でも動かさない**（計算開始時に必ず希望へ戻す案と、手で決めたかを記録する案は不採用）。入口の hf67 は「良ければ採る」のまま。
    希望と同じ値のときだけ守る段（`CovOReliefPolish`・`RsiHypothesisOperators` 4 か所・hf66 の上限 0 の置き換え）を同じフラグで値によらず守る修正を追加中。
