...
29. **[要grilling・高] `SmartInitialScheduler.solveConstructionDp`の状態爆発（窓長=31日で最大2^30状態、
    上限/打ち切りなし）**（外部提案、2026-09-17・サブエージェントで実コード確認済み）。状態キーは
    直近`maxWindow-1`日のビット履歴＋累積回数で、希望固定/個人上限が無いと状態が併合されず窓長に
    対し指数的に増える。`ConstraintMus.cachedMinDays`もキャッシュ前の初回計算で同じ爆発に晒される
    （セル編集のたびに呼ばれる`analyzeStaffConflicts`経由）。対策は長い窓向けの別解法・状態圧縮と
    計算資源の上限、打ち切り時は「充足不能」と誤判定せず未完了として扱う設計。着手前にgrillingで
    対象範囲（窓長の上限値・打ち切り後の扱い）を確定する。

30. **[要修正・中] `MinCostAssignment.solve`が禁止辺(INF)を含む割当を返しうる**（外部提案、2026-09-17・
    2x2ケースで実際に潜在値補正の算術をトレースし確認済み）。Hungarian法の潜在値(u/v)補正でINFから
    減算する際に有限値へ「腐食」し、完全実行可能割当が存在しない場合でも禁止辺を含む割当を返す
    （nullを返すべき所）。呼出元`DayAssignmentPolish`は結果を必ず`UnifiedViolationChecker`+
    `adoptionGate`（keep-best）へ通すため最終採否では捕捉される＝実害は未確認だが、ソルバー自体の
    契約違反。修正は禁止辺を探索から除外する（potential更新に触れさせない）か、返却前に禁止辺を
    含まないことを検証する。
...
34. **[要調査・未解決の既知課題] ロールの時間クォンタム超過＝根本原因は3.409.17時点で未特定のまま**
    ...
    明記され再現も失敗している。今回のサブエージェント調査で新たな手がかりを発見: `nativeSaChunk`
    （SA用ネイティブチャンク、`magi_native.cpp`）だけがALNS/Polishのチャンクと違い明示的な反復回数を
    引数で渡さず、内部の`maxIters=200000`（他の約500倍）のみで制御され、チャンク実行中は壁時計/
    `stopRole`の確認が一切ない（`SaOptimizer.kt`の`timeUp()`はチャンク間でしか効かない）。既定
    パラメータでは1チャンクは数千反復で高速なため、これだけで過去に観測された超過を説明できるかは
    未確認。着手前にgrillingで検証方法（合成の長時間チェーン条件の再現・計装追加）を確定する。
