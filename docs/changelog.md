...
> このファイルは `CLAUDE.md` から切り出した、版ごと（3.xxx.0 単位）の詳細な変更履歴。
> 現在有効なルール・基本方針は `CLAUDE.md` 本体を参照。ここは個別の修正内容・調査記録・実測値の
> アーカイブで、通常のセッション開始時には読み込まれない（必要な版の記述を検索して読む用途）。
> 分割時点（3.409.29 相当）までの全エントリを原文のまま収録。以降の新規エントリは、works content的に
> 「現在の版数と直近の教訓」程度の要約だけ CLAUDE.md 側に足し、詳細はこちらへ追記していく運用とする。
