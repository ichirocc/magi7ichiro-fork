...
- **上から順に**（読む所が上、押す所が親指の届く下）:
  ...
  3. 違反があれば直し方（`FixSearchPanel` の compact＝手 1 件か、手が無い理由 2 行＋［希望を見る］［設定を見直す］）と「ルールの見直しへ」（旗のアイコン）。
     **希望を守っている板挟み**（希望どおりのセルに違反）: 「本人の希望（休）を守っています」＋原因の 2 行、［他の人で補う（推奨）］＝本人を動かさない
     その日の手だけを探す（`FixFocus.exceptStaff`・`fixesByOthers`）、［希望を取り消して別のシフトを割り当てる］＝割当のボタンを出す（希望は登録のまま＝
     置けば希望違反として見える。黙って崩さない）。選ぶまでシフトボタンは出さない。
...
- **押したら即反映**: 割当は `setCell`、Snackbar「氏名 3日をA4に変更しました｜元に戻す」（1 段戻す、`UiState.undoableMessage`）。確認ダイアログは出さない。
