...
- **思考誘導カード（最上部・operator_ux §3）**: `OperatorNextActionCard`。いまの状態で文言・色・大ボタンが自動で変わる：未作成→「② 勤務表をつくる」／組立中→「あと約N分・閉じても大丈夫（やめる）」／配れる(緑)→「そのまま配れます（印刷・書き出し）」／もう少し(黄)→3.612.0 から「足りる？→1手ある？→希望が関わる？」の順に1つ＝今すぐ直せる人手不足「なおすのを手伝って」／1手あり「直す1手を見る」／探索中は見出しのみ／希望が関わる必須（pref/c3w）「ぶつかっている希望を見る」（`WishConflictDialog`＝関わる希望を名前・日付・理由つきで列挙、押すとそのセルを開く）／1手が無く長く改善しない必須族が残り希望が関わるときだけ「…下限の見込みです」＋「ぶつかっている希望を見る」（補助「このまま書き出す」）／それ以外「問題を見る」／埋められない(赤)→「ここは埋められません（データを見直す／そのまま配る）」。**大ボタンは常に1つ**＋控えめな補助1つ。数字は言葉つき（人手が足りない日／解消度）。専門記号は出さない。
