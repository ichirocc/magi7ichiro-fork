...
- **上から順に**（読む所が上、押す所が親指の届く下）:
  ...
  7. 幅広の「閉じる」（希望モードで登録済みなら隣に「希望を取り消す」。閉じるが利き手の側）。その下 32dp は押せるものを置かない（ジェスチャー帯の上のクッション）。
...
- **実装**: `CellEditSheet.kt`（画面）／`CellSheetLogic.kt`（固定配置・1 行の状態・印・巡回・回数の 1 行＝ホストテスト `CellSheetLogicTest`）／`setCell`・`setWish`・`removeWish`。
