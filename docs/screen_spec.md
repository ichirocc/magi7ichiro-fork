...
- **希望シフトの表示（割当との融合）**: 各セルの**右下に希望の小バッジ**を重ねて表示。**緑＝反映済（割当＝希望）／桃＝未反映（割当≠希望）**。希望が登録されていないセルにはバッジを出さない。読み上げにも「・希望{記号}（反映済/未反映）」を付与（色覚多様性・スクリーンリーダー対応）。**7日表示と1ヶ月マトリクスの両方**に適用（共通 `Cell`）。これにより「割当」と「希望」を1枚の表で同時に把握でき、未反映（桃）が一目で分かる。
...
- **セル系**（c1/c3/c3n/c3m/c3mn/c42/c42s/pref/groupViol）＝`violationCells`(i,j)。**勤務表のセル枠/ドットで直接可視化**：HARD＝実線枠＋塗りドット／SOFT＝破線枠＋中空リング。チップタップは「スタッフ名 日付」。
