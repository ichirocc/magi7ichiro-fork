...
- **違反の表示（3.612.0）**: このセルの族（職員×日）と今のシフトの人員の族（シフト×日: covU/covO/c41/c41s）をまとめ、「必須（配る前に直す）」→「要調整（できれば）」の見出しで分けて重い順に並べる。必須があるときは要調整を「要調整 N件（後回しで大丈夫）▸」に折りたたむ（タップで開閉）。回数系（c2/low/high/apt）と位置なし（fair/weekly）はセルに出ない（集計表・内訳側）。
