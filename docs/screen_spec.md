...
- **本書の反映バージョン**: app **v3.35**（編集タブ=3サブタブ[今月の調整／シフト希望／基本マスター]・基本マスターは5節に集約・**分析タブに一般/プロ切替＋違反内訳19/19=100%(fair/weekly含む)**・**全ポップアップ統一[`DialogHeader`タイトル＋右上✕／共有3ボタンで確定=右・取消=左・48dp・危険⚠]**・全ダイアログ縦スクロール・コンポーネント別CSV取込/スタッフupsert・氏名照合の空白無視・グループ削除の自動再割当・編集の自動保存＋背景即時flush・設定タブの冗長除去＋見出し体裁の統一）。
  - **v3.8→v3.35 のUI差分**: ①編集タブ再設計（月次/年次マスターの2分割→**今月の調整／シフト希望／基本マスター**の3分割、年次の7節→**5節に統合**）。②分析タブに **一般/プロ** 切替を追加し、熟練者向けプロ表示では冗長な説明文を非表示。違反内訳に **fair（公平化のズレ）** を加えて全19種を表示（fair・weekly 追加で **19/19**）。③ポップアップ全種（ダイアログ／フォーム／ボトムシート／ピッカー／確認）を統一：ヘッダーは `DialogHeader`、ボタンは共有3部品で**確定=右・取消=左**を固定、最低48dp、破壊的操作は**⚠＋エラー色**。色設定の左右逆ボタンを是正。
...
| 13 | エラー・空状態・結果通知（共通規則） | 規則 | （図なし。Snackbar＋操作ログ） |
...
| 領域 | Web版 | Native版 |
|---|---|---|
| セル編集 | グリッドセルをクリックして**値を入力/選択**（Excel式） | tap→**CellEditSheet**（画面下の固定パネル）で固定配置のシフトボタンから選択（自由入力不可・誤値防止）＋触覚 |
...
| 進捗 | BusyOverlay（反復中心）＋**TapGame**（ミニゲーム） | `CopilotCard`（満足度/残時間/未解決）＋赤「やめる」。TapGameは**非採用** |
...
- **構成**: `MagiSegmentedControl(['今月の調整','シフト希望','基本マスター'])`（`MagiApp.kt` の `editScope` 0/1/2）。
  ...
  - **基本マスター**(2) = 注意帯（「制度・人員が変わったときだけ編集」）＋**5つの折りたたみ節**(`CollapsibleSection`)。各節は先頭に平易な一文(`SectionNote`)を持ち、`rememberSaveable` で開閉状態を保持して縦長を畳む。
    ...
    - **⑤ 並び・期間の制約** = `ConstraintsCard`（cons1/2/3/3n/3m/3mn：並び・窓）
- **ラベル混同防止**: サブタブ「シフト希望」(ws3＝勤務の希望) と ⑤ 内の cons3系(ws4＝並びパターン) を区別するため、cons3m を**「並び希望」**、cons3mn を**「並び回避」**と表記（cons3=「守るとよい並び」/cons3n=「禁止の並び」）。
...
- **主要オブジェクト**: 見出し「勤務表データを開きましょう」、`データを開く`(primary 56dp)、`サンプルで試す`(outline 56dp)、
  `新規につくる（空から）`(outline 56dp＝最小データで開始し編集タブ＞基本マスターへ)。
- **メッセージ**: 「保存済みのデータを開く、サンプルから始める、または空から新しく作れます。」
- **指1本**: 大ボタン3つを画面中央〜下に縦積み。OSファイルピッカー（SAF）を起動。
- **実装**: `EmptyStateCard`（`MagiApp.kt`）。未読込のあいだは**設定タブ以外のどのタブでもこのカード**（`!ui.loaded && tab != 4`）。
  上部チップと下部コマンドバーは読込前は出さない。失敗・空状態の扱いは §13。

## 01. ホーム
![ホーム](screens/01_home.png)

- **目的**: 「配れるか・どこが問題か・次に何をするか」を一画面で提示。
- **思考誘導カード（最上部・operator_ux §3）**: `OperatorNextActionCard`。いまの状態で文言・色・大ボタンが自動で変わる：未作成→「② 勤務表をつくる」／組立中→「あと約N分・閉じても大丈夫（やめる）」／配れる(緑)→「そのまま配れます（印刷・書き出し）」／もう少し(黄)→3.612.0 から「足りる？→1手ある？→希望が関わる？」の順に1つ＝今すぐ直せる人手不足「なおすのを手伝って」／1手あり「直す1手を見る」／探索中は見出しのみ／関わる希望がある（必須に関わる希望、または人手不足の日に別の勤務の希望＝S5）「ぶつかっている希望を見る」（`WishConflictDialog`＝関わる希望を名前・日付・理由つきで列挙、押すとそのセルを開く。各行の「取り消したら？」で試算し「希望を取り消して、もう一度つくる」で確定＝`docs/s5_wish_trial.md`）／1手が無く、背景の試算（S6、起点 3 件まで・［やめる］で止まる）が上限 0 の組で必須を減らせると見つけたときだけ、希望の段より先に「設定が壁になっています」＋「緩める候補を見る」（`RelaxTrialDialog`＝前提・組・手順を言葉で出し、「この組で緩めて、手順を当てる」で上限 0→1 と手順を Undo 1 段で当てる。仮の盤面は出さない＝`docs/s6_relax_trial.md`。補助は関わる希望があれば「ぶつかっている希望を見る」）／1手が無く長く改善しない必須族が残り希望が関わるときだけ「…下限の見込みです」＋「ぶつかっている希望を見る」（補助「このまま書き出す」）／それ以外「問題を見る」／埋められない(赤)→「ここは埋められません（データを見直す／そのまま配る）」、人手不足の日に別の勤務の希望がある人がいれば「いまの希望のままでは、ここは埋められません（ぶつかっている希望を見る／データを見直す）」。**大ボタンは常に1つ**＋控えめな補助1つ。数字は言葉つき（人手が足りない日／解消度）。専門記号は出さない。
...
- **冗長性削減（QA）**: ホームは思考誘導カードを主役に整理。`StatusHero`（状態の三重表示）・`SummaryCard`（統計は「ようす」と重複＋開発用語 2世代カバレッジ/初期HARD等）・`QuickActionGrid`（下部ナビと4/6重複）を**除去**。`CopilotCard` は 解消度バー/進捗の重複を外し**助言・警告だけ**に。`ActionCard` は主操作を外し**「ほかの作り方（速く/かんたん）＋閉じても続ける」**に縮小。下部CTA「勤務表をつくる」は思考誘導カードと**同一動作（本最適化）に統一**（同名ラベル別動作の不整合を解消）。
...
- **目的**: 最適化中の進捗を可視化し、安心して待てる/止められる状態にする。
- **主要オブジェクト**: 状態チップ=実行中（青）、スピナー＋「残り約 N 秒 ・ 未解決 ・ 反復」、進捗バー、下部 `■ やめる`（赤）。
...
- **1つのセルに表示される情報（重ねて最大4層）**: ①**背景色**＝シフト色 ②中央の**シフト記号**（空欄＝未割当）③**違反枠**（HARD実線/重いSOFT破線、軽いSOFTは右上の角マーク・12dp）④**希望バッジ**右下（緑リング＝反映済/桃バッジ＝未反映、中に**希望の記号**を白文字）。読み上げ＝「シフト{記号}・{必須違反/要調整}・希望{記号}(反映済/未反映)、タップで変更」。
- **違反表示（非色手がかり）**: HARD=**実線枠(3dp)** / 重いSOFT=**破線枠** / 軽いSOFT=**右上の角マーク**（色覚多様性・モノクロ印刷対応）。凡例「実線＝必須違反 / 破線＝要調整」。違反種別の意味・重み・場所の一覧は §08b。
- **表示専用の印（2026-09-25）**: チェッカーの場所マップ（探索も読む）は変えず、`MagiViewState` が報告から別に組み立てる。
  ①回数の族（low/high/apt/c2）＝**職員名の横の ▼/▲**（名前の枠 48dp をタップで「回数・偏りの内訳」＝公平化・曜日の偏りも `distLocations` から列挙。「回数」チップで絞る）
  ②人員（covU/covO）＝**日付の下にシフト名＋向き**（例「休▲」、複数は「+N」。旧「▼N ▲N」の人数表示を置き換え。日ヘッダをタップで日の人員の内訳。「人員」チップで絞る）
  ③期間の制約（c1）＝**不足窓の中で、いまそのシフトでなく、そのシフトに変えられる日**（担当可・`mayPlace`・希望固定は希望どおりのときだけ）に破線の印
  （`c1Shortages`＝`ui/C1Display.kt`。すでにそのシフトの日・チェッカーのランの先頭には描かない。どのシフトの規則でも同じ）。窓が重なって続く不足は
  **行の下に細い帯**（控えめの色、「期間の制約」チップで絞る）。セルシート・詳しくは「期間の約束: {day1}日のなかに「{記号}」が{day2}日必要です。いま足りない期間
  （{from}〜{to}）があり、印の日を{記号}にすると届く見込みです。」＋すでにそのシフトの日は「（この日の{記号}はすでに数に入っています）」。変えられる日が 1 つも無い
  不足窓は印を出さず「希望や担当の都合で、勤務表だけでは期間の約束を満たせません。」と［希望を見る］［設定を見直す］（セルシートと職員名の内訳）
  ④最重の族に隠れた別の族があるセルは**左上に小さな点**
  ⑤セルシートに「この職員の回数・偏り」。フィルタのチップは 人員／希望／**並び**／回数／グループルール／期間の制約。
  ⑥違反セル・印・集計の内訳のシートは**開いた時点でその対象の直し方を探し**、同じシートに手（「この手を使う（元に戻せます）」）か、0 件なら確かめた理由と「希望を見る」「設定を見直す」を出す（盤面が変われば探し直し、閉じれば取り消し）。
  ⑦シフト集計の「計（期間）」はそのシフトの人員不足▼・過剰▲の日数を添え、タップで日の一覧。
  凡例は形ごとの族名 1 行（「実線: 禁止の並び・希望前日の禁止・希望違反・担当外シフト／破線: 期間の制約・回避の並び」）を添える。
...
## 04. セル編集シート（手動修正の主操作、2026-09-25 再設計）
![シフト選択](screens/03_picker.png)

- **目的**: タップしたセルの割当・希望を、勤務表を見たまま変える。
- **形**: 画面下の**固定パネル**（`CellEditSheet`、モーダルでない＝勤務表は上で見えて押せる）。**引っ張る・スワイプの操作は無い**（片手一本指）。
  閉じる＝「閉じる」／ヘッダーの✕／戻る／勤務表の何も無い所のタップ。**別のセルをタップするとそのセルへ移る**（閉じて開き直さない）。
  開いたセルはシートの直上へ縦横に自動で寄せ、行と日ヘッダを強調し、セルに太い枠（前のセルの枠は残らない。窓の強調は同じ職員の行だけ）。
- **上から順に**（読む所が上、押す所が親指の届く下）:
  1. 見出し「氏名 ・ 10/3(土)」＋（巡回中は「次の違反 ▶」）＋✕。
  2. **1 行の状態**（`cellStatusLine`）: 必須＝淡い赤＋赤のアイコン、要調整＝淡いアンバー＋橙、違反なし＝淡い灰（`magiSeverityColors`）。最も重い族について
     原因と関わる人・日・数をチェッカーと同じ盤面から言う（c3w「翌日(休)への前日禁止（Dﾃ）」、c3n「禁止の並び Dﾃ→A4（10/8〜10/9）」、
     c42s/c42「職員04(休)とのスキルグループペア禁止」、covU/covO「10/9のA4が人員過剰（適正2人に3人）」、c1「期間の制約：10/1〜10/7にAｱが0回（1回以上）」、
     low/high/apt「Aｱが7回（適切8回）」ほか）。ほかの族は「（ほかN件）」。
  3. 違反があれば直し方（`FixSearchPanel` の compact＝手 1 件か、手が無い理由 2 行＋［希望を見る］［設定を見直す］）と「ルールの見直しへ」（旗のアイコン）。
     **希望を守っている板挟み**（希望どおりのセルに違反）: 「本人の希望（休）を守っています」＋原因の 2 行、［他の人で補う（推奨）］＝本人を動かさない
     その日の手だけを探す（`FixFocus.exceptStaff`・`fixesByOthers`）、［希望は残して別のシフトを割り当てる（希望は未反映になります）］＝割当のボタンを出す（希望は登録のまま＝
     置けば希望違反として見える。黙って崩さない。希望そのものを消すのは希望モードの「希望を取り消す」）。選ぶまでシフトボタンは出さない。
     直し方の状態は 計算・チェック待ち／未開始／探索中（スピナーはここだけ）／完了／失敗（［もう一度探す］）＝`fixPanelState`。チェックが終わった時点と、
     盤面・希望・設定が変わったときに探し直す。
  3b. ［詳しく ▼］で開く節: 「このセルの違反」＝セルに重なった族をすべて重い順に「必須・…」「要調整・…」（期間の制約は「期間の約束: …」の文、
     `cellDetailLines`）、「この職員の回数・偏り」（`staffCountLines`、行末の印のシートと同じ）。
     状態の 1 行・印・回数の 1 行・詳しくは同じ版（`cellSheetRev`＝盤面・希望・設定 editRev・検査世代）から作り直す＝希望だけ変えても古い理由や印が残らない。
  4. 1 行「［割当］［希望］ 希望 休（反映済）　回数 Aｱ 7(適8)▼」。切替の 2 つだけがボタンで、補足（希望の中身・回数＝`staffCountShort`）は控えめの色
     （割当の値は選んだボタンの ✓ が示すので書かない）。
  5. ［◀ 7日(水)］［9日(金) ▶］＝同じ職員の前日・翌日へ（シフトボタンの直上、利き手の側に寄せる）。
  6. **シフトボタン（固定配置）**: シフト一覧の順・4 列・全幅・52dp。**データの中で誰も担当できないシフトは出さない**（配置はデータごとに 1 回＝`sheetShifts`）。
     職員・割当/希望の切替で**位置が動かない**。担当外も枠に残して灰色＋「外」
     （割当では押せない＝従来どおり、希望では登録できる）。選んでいるシフトは自分の色のまま太枠＋✓。印（ボタンの右上の角、形と色だけ）: 緑の点＝おすすめ（必須のセルは必須が減り
     どの必須族も増えない、要調整だけのセルは重み付き合計が減り必須が増えない）、赤の警告＝置くと必須の族が増える（`evaluateShiftMarks`、
     チェッカーで候補ごとに評価・背景スレッド・セルが変われば取り消し）。割当では希望のシフトに小さく「希望」。
  7. 幅広の「閉じる」（割当モードでは隣に［固定する］／［固定を外す］＝手動固定の付け外し、希望モードで登録済みなら隣に「希望を取り消す」。閉じるが利き手の側）。その下 32dp は押せるものを置かない（ジェスチャー帯の上のクッション）。
     **手動固定**（backlog #41）: いまの値のまま最適化器に書き換えさせない。付け外しは元に戻すの 1 段、Snackbar「氏名 3日を固定しました（最適化で変わりません）」。
     固定中は 4. の補足に「・固定中」、勤務表のセルの**右下に小さな錠**（`MagiMarks.pinLock`、希望の印は左下の丸・バッジ＝位置と形で区別）。
     手で割当を変えてよく、固定は残って値が変わる。固定セルに違反が残れば 1 行の状態の末尾に「手動固定のため直せません」。
- **利き手**（設定タブ＞外観「セル編集シートの利き手」右手／左手、既定＝右手）: 左手ではシフトボタンの各行を左右反転（空きも含めて反転＝同じシフトは同じ枠）、
  日送りは左寄せ、閉じるは左。
- **勤務表の印の強弱**: 必須の実線枠は 3dp のまま、要調整の破線は 2dp・角の印は 10dp・名前の横の ▼▲ は細字で要調整の色（`MagiMarks`）。
- **押したら即反映**: 割当は `setCell`、Snackbar「氏名 3日をA4に変更しました｜元に戻す」（`UiState.opNotice`＝検査の進み具合の文言とは別のイベントで、
  出ている間は再検査の文言で置き換えない。「元に戻す」はその操作が元に戻すの先頭にあるときだけ効く＝`noticeUndoApplies`）。確認ダイアログは出さない。
- **巡回**: 上部バッジ「必須違反 N件を順に見る」→ 必須のセル（日→職員の順、`violationTour`）を開き、シートの「次の違反 ▶」で進む。必須が 0 件になると要調整のセルを巡る。
  人員・回数の族はセルを持たないので巡回に入らない（日ヘッダ・名前の横の印から開く）。
- 撤去した文言: 「必須（配る前に直す）」「要調整（できれば）」「タップで割当を即変更。」「外＝担当外…」、「希望どおり X にする」（希望のボタンに「希望」の印）。
- **実装**: `CellEditSheet.kt`（画面）／`CellSheetLogic.kt`（固定配置・1 行の状態・印・巡回・回数の 1 行＝ホストテスト `CellSheetLogicTest`）／`setCell`・`setWish`・`removeWish`・`togglePin`（`ManualPinTest`）。
...
- **希望の一括操作**: 「希望シフトの一括操作（曜日／全体）」→ボトムシート(`WishBulkSheet`)で曜日・全体まとめて希望を設定/消去。全職員へ付けるときは選んだシフトを担当できる人だけ（「N名 × D日 = M件（担当外 X名は対象外）」と表示。担当外の人の既存の希望は変えない）。
...
| キー | 表示名 | 重み | 場所 | 意味 |
|---|---|---|---|---|
| `groupViol` | グループ不整合 | 11000 | セル | グループ制約に反する配置 |
| `covU` | 人員不足 | 10000 | 日付 | その日のシフトの必要人数に足りない（被覆不足） |
| `c3n` | 禁止の並び | 9000 | セル | 禁止された勤務の並び（cons3n／FORBIDDEN） |
| `c3w` | 希望前日の禁止 | 9000（c3n と同格、3.542.0） | セル | 希望で固定されたシフトの前日に置けないシフト（cons3w）。違反箇所は前日側のセル |
| `pref` | 希望違反 | 8000 | セル | 登録された希望シフトが満たされない |

### SOFT（できれば・15種）※重み降順
| キー | 表示名 | 重み | 場所 | 意味 |
|---|---|---|---|---|
| `low` | 下限割れ | 120 | 回数 | 個人の回数が下限(ws5)を下回る |
| `c3mn` | 回避の並び | 90 | セル | 避けたい勤務の並び（cons3mn／編集名「並び回避」） |
| `c1` | 期間の制約 | 50 | セル | 一定期間（窓）内の要件（cons1）。分析タブの要確認一覧のバッジは短縮の「期間」 |
| `high` | 上限超過 | 25 | 回数 | 個人の回数が上限(ws5)を超える |
| `c3` | 守るとよい並び | 15 | セル | 望ましい勤務の並び（cons3）※Androidでは SOFT 扱い |
| `c41s` | スキルグループのレンジ | 10 | 日付 | スキルグループの人数レンジ（C41s） |
| `c42s` | スキルグループペア | 10 | セル | スキルグループの禁止ペア（C42s） |
| `covO` | 過剰な配置 | 10 | 日付 | 必要人数を超える配置（被覆超過） |
| `c41` | グループのレンジ | 9 | 日付 | 1日あたりのグループの人数レンジ（C41） |
| `c42` | グループペア | 9 | セル | 同日共存を禁じるグループペア（C42） |
| `c3m` | 推奨の並び | 6 | セル | 推奨の勤務の並び（cons3m／編集名「並び希望」） |
| `c2` | 個人の合計 | 4 | 回数 | 個人の合計回数の要件（cons2） |
| `apt` | 適切回数のズレ | 4 | （無し） | 目標回数(ws1 C／groupShiftApt)とのズレ |
| `fair` | 公平化のズレ | 2 | （無し） | 職員間の偏り（公平性） |
| `weekly` | 曜日の偏り | 2 | （無し） | 7日周期（曜日）のシフト回数の偏り（休も1シフトとして数える） |

### 違反箇所（場所）の出し方（`breakdownLocations`）
場所は3系統で算出し、内訳チップのタップで一覧へ飛べる：
- **セル系**（c1/c3/c3n/c3w/c3m/c3mn/c42/c42s/pref/groupViol）＝`violationCells`(i,j)。**勤務表のセル枠/ドットで直接可視化**：HARD＝実線枠＋塗りドット／SOFT＝破線枠＋中空リング（**ドットは7日表示の `Cell` のみ。1ヶ月マトリクスは枠のみ・カレンダーは日別の不足表示**）。チップタップは「スタッフ名 日付」。
- **回数系**（low/high/c2）＝`countViolations`(i,k)。チップタップは「スタッフ名「シフト」」。
- **被覆系**（covU/covO/c41/c41s）＝`needViolations`(k,j)。チップタップは「日付「シフト」」。
- **場所なし**（apt/fair/weekly）＝セル単位の位置を持たないため、タップ時は「場所情報がありません」（ペナルティ量のみ表示。weeklyは内訳チップに件数のみ表示）。
...
- **CSV取込（強化）**: 取込時に**文字コードを自動判定**（妥当な UTF-8 はそのまま／不正なら CP932(Shift-JIS) として復号。日本の Excel 由来CSVの文字化けを解消）。さらに**病院などの「勤務表テンプレCSV」を新規データとして丸ごと取込**（`RosterCsvImport`）。**固定セル対応**：年月タイトル→期間、グループ名＝**C2/C13**（ユニット見出し）、氏名＝**C4:C11/C15:C22**、勤務記号＝本表**E4:AI11/E15:AI22**（31日）、シフト記号＝凡例**B25:B40**（時刻はC列＝表示名）。**必要人数(need1/need2)はこのCSVに存在せず取り込まない**（凡例の日別数値は現在表の人数集計＝需要ではないため、休/有の人数も含む）。担当可否は不明のため全シフト可で取り込み、後から調整可。
  ...
  - **コンポーネント別の取込/出力**: データ全体だけでなく、**スタッフ一覧／希望シフト／各制約／勤務表／シフト色**を種別選択して個別に取込・出力できる（`StaffCsvIO`/`WishesCsvIO`/`ConstraintsCsvIO`/`ShiftColorsCsvIO`）。**スタッフ一覧は upsert**＝既存氏名は所属グループ/スキルを更新、未知の氏名は**新規スタッフとして追加**（勤務表に休の行を追加）。そのため別形式のCSV（勤務表・テンプレ・希望・制約・シフト色の見出し）は取込の前に断り、正しい取込種別を案内する。**シフト色（3.547.0）も upsert**＝「記号,色」の2列で、違反色（`__vio__`等の予約キー）は対象外＝シフト種別のカスタム色のみ。CSVに載っている記号だけ`shiftColors`を更新し、載っていない記号の既存カスタム色は変更しない。現在のシフト一覧に無い記号・不正な色形式は反映せず件数で案内。
...
## 13. エラー・空状態・結果通知（全画面共通の規則）

- **経路は2つだけ**: **状態**（実行中／配布可／必須違反 N／未計算）は上部チップと進捗行、**出来事**（保存した・読めなかった・
  止めた）は **Snackbar**（下部コマンドバーの上、押した場所の近く。3.399.0）。本文の中にメッセージ行を置かない。
  実体は `ui.message`＋`ui.messageIsError`（`MagiUiState`）。失敗は `errorContainer` 色で **Long**（120字級を読み切れる長さ）、
  それ以外は `inverseSurface` 色で **Short**。表示し終えたら compare-and-clear（`clearMessage(shown)`）＝同じ文言が
  続けて来ても次のタップでもう一度出る。**失敗は必ず操作ログにも W で残す**（`notify`／`logOp`。設定＞詳細設定＞ログ）。
- **文言の規則**（3.147.0／3.191.0／3.400.0）: 画面には「**何ができなかったか＋次の一手**」だけ。生の例外文（`e.message`）と
  内部名（V6・エンジン名）は出さず、種類名（`javaClass.simpleName`）を括弧で添えるにとどめ、詳細は操作ログへ。
  ファイル入出力の理由は `ioReason` で利用者の言葉に置き換える: 内容が空でした／アクセスが許可されていません／
  ファイルが見つからないか、書き込みが許可されていません／保存先の空き容量が足りません／それ以外は種類名。
- **保存（JSON・CSV・ログ出力）**: **成功も必ず通知**「◯◯を保存しました」（旧: 成功時に無反応で 0 バイトのファイルだけ残る
  事故が起こりうるため、3.400.0）。失敗は「◯◯を保存できませんでした（理由）」＝W。`notifySave`。
- **開く（JSON）**: 「読込中…」→ 成功「読込完了: N名 / N日 / Nシフト」／失敗「読み込めませんでした（種類）。ファイルの中身を
  確認してください」＝W／中止「読み込みを中止しました」＝**失敗ではないので通常色**（3.404.0）。開く系の失敗は
  `notifyOpenFailure`「◯◯を開けませんでした（理由）」。**OS ピッカーのキャンセル（uri=null）は何も出さない**。
- **CSV 取込の失敗は4型**（`MagiViewModelIo`）: ①**形式が読めない**＝直し方まで書く（凡例が無い→「テンプレCSV末尾の
  『記号 / 時刻 …』一覧が削除されていないか…Excelで保存すると消える」、引用符が閉じていない→書式を直す、ユニット列が
  解析不能→ヘッダ行と氏名列）。②**前提不足**「先にデータを開いてください（◯◯は既存データに重ねます）」。③**0件**
  「◯◯の取込失敗（取り込める行が0件）」＋`componentImportMismatchHint`（勤務表/ロスター形式を希望・制約として
  選んだ場合は正しい種別を案内）。④**一部の行が読めない**→「◯◯の取込を中止しました（読めない行がN件）」＝
  **部分適用しない**（希望・各制約は all-or-nothing）。シフト色（upsert）は未知の記号・不正な色形式を**件数で案内**し
  読めた分だけ反映。
- **最適化の失敗・停止**: 例外（`Error` を含む）→ `running=false` に戻し「勤務表をつくれませんでした（種類）。もう一度
  お試しください（詳しくは設定＞詳細設定＞ログ）」（仕上げは「整えられませんでした（…）」）。**やめる**→
  「停止しました。直前の勤務表（必須=N 合計=N）を保持しています。」＝通常色、盤面は直前の最良を保持。
  実行中の編集・設定変更は拒否＝W「◯◯の実行中は…できません。終わるか「やめる」を押してからにしてください。」。
  セル編集ごとの違反チェックが落ちたら「違反チェックに失敗しました（種類）」＋ログ（画面は読取専用にならない、3.392.0）。
  プロセス kill は §11 のバナー（入力は自動保存済み・途中結果から再開）。
- **空状態**: データ未読込＝§00 のカード。結果なし＝上部チップ「未計算」、分析タブのバッジ「未計算」と
  見出し「エンジンが挑戦する項目（未計算）」。**古い結果の失効**（盤面や設定が変わった後）: 他の案は
  「…この案は適用できません。もう一度最適化してください」、直し方の提案は「…「直し方を探す」をもう一度押してください」＝
  黙って適用しない。
- **実装**: `MagiApp.kt`（`SnackbarHost`／`LaunchedEffect(ui.message)`）、`MagiViewModel.notify/notifySave/notifyOpenFailure/
  ioReason/clearMessage`、`MagiViewModelIo.kt`（取込 4 型）、`EmptyStateCard`。
...
### A.1 アプリシェルとナビゲーション
- **構成**: 最上部 `MagiTopBar`（ロゴ＋状態チップ＝未読込／実行中(青)／配布可(緑)／要確認(黄)）、本文、最下部に **コマンドバー** `BottomCommandBar`（主操作「最適化する」＋控えめな補助1つ。**実行中は「■ やめる」(赤)** に差し替え）と **5タブ** `MagiBottomNav`（**ホーム / 勤務表 / 編集 / 分析 / 設定** ＝ tab 0..4）。
...
| `cons3w` | List\<C3wRow\> | 希望の前日に禁止（3.542.0）。編集タブ ⑤ 並び・期間の制約 に族「希望の前日に禁止（必ず守る）」 |
...
- **I/O**: JSON開く/保存はOSファイルピッカー（SAF）。CSVは文字コード自動判定（妥当UTF-8はそのまま/不正はCP932復号）、勤務表テンプレCSVの丸ごと取込（勤務表/希望を選択）、コンポーネント別取込/出力（スタッフは upsert・氏名照合は空白/先頭BOM無視）。固定セル仕様と詳細は§09、失敗時の通知は§13。
...
```json
{
  ...
  "shiftColors": { "日": "#2E9E62", "夜": "#E08A1E", "休": "#8A979B", "__vio__": "" }
}
```
...
5. 希望バッジ＝**右下**(`BottomEnd`)・padding1dp・角丸4dp・1dpのsurface縁・横padding3dp・中に希望記号（`labelSmall`・白・1行）。緑リング＝反映済／桃 `#D24D89`＝未反映。
