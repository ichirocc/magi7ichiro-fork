...
MAIN_SRC=$(find "$A/v6" "$A/model" -name '*.kt'; ls "$A"/ui/{MagiUiState,AnalysisTriage,BreakdownLabels,ConstraintHelp,VioBuckets,MagiPhase,MagiEvent,MagiMediator,MagiViewState,MagiConstraintsView,MagiWs1View,MagiConditionsView}.kt "$A"/work/{RunFiles,RunMarker,SaveGate,OptimizationRepository}.kt "$HERE"/stubs/*.kt)
