...
OUT=$HOSTOUT/s5probe; rm -rf "$OUT"; mkdir -p "$OUT"
java -Xmx2g -cp "$KC" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler -nowarn -no-stdlib -no-reflect -jvm-target 17 -Xfriend-paths="$HOSTOUT/main" -cp "$CP" -d "$OUT" "$HERE/LoopBench.kt" "$HERE/S5Probe.kt" 2>&1 | grep -E "^e: |error:" && exit 1
java -Xmx3g -Dfile.encoding=UTF-8 -cp "$CP:$OUT" probe.S5ProbeKt "$1" "${2:-30}" "${3:-3}" "${4:-6}" "$ROOT/app/src/test/resources" 2>&1 | grep --line-buffered -v JAVA_TOOL
