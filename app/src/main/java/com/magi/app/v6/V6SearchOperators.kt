...
internal fun findCovOFix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    var wCnt = 0
    for (i in 0 until p.S) if (eval.at(i, j) == overK && !p.wishLock[i][j]) wCnt++
    if (wCnt == 0) return null
    var pickW = rng.nextInt(wCnt); var i = 0
    for (ii in 0 until p.S) if (eval.at(ii, j) == overK && !p.wishLock[ii][j]) { if (pickW-- == 0) { i = ii; break } }
...
internal fun findC2Fix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    var dayCnt = 0
    for (j in 0 until p.T) if (eval.at(stf, j) != c.shiftIdx && !p.wishLock[stf][j]) dayCnt++
    if (dayCnt == 0) return null
    var pickJ = rng.nextInt(dayCnt); var day = 0
    for (j in 0 until p.T) if (eval.at(stf, j) != c.shiftIdx && !p.wishLock[stf][j]) { if (pickJ-- == 0) { day = j; break } }
...
internal fun findRangeLowFix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    var dayCnt = 0
    for (j in 0 until p.T) if (eval.at(rlI, j) != rlK && !p.wishLock[rlI][j]) dayCnt++
    if (dayCnt == 0) return null
    var pickJ = rng.nextInt(dayCnt); var day = 0
    for (j in 0 until p.T) if (eval.at(rlI, j) != rlK && !p.wishLock[rlI][j]) { if (pickJ-- == 0) { day = j; break } }
...
internal fun findC41Fix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    return when {
        cnt > c.u -> {
            var wCnt = 0
            for (i in 0 until p.S) if (p.sgrp[i] == c.groupIdx && eval.at(i, j) == c.shiftIdx && !p.wishLock[i][j]) wCnt++
            if (wCnt == 0) return null
            var pickW = rng.nextInt(wCnt); var ci = 0
            for (i in 0 until p.S) if (p.sgrp[i] == c.groupIdx && eval.at(i, j) == c.shiftIdx && !p.wishLock[i][j]) { if (pickW-- == 0) { ci = i; break } }
        ...
        cnt < c.l -> {
            var aCnt = 0
            for (i in 0 until p.S) if (p.sgrp[i] == c.groupIdx && eval.at(i, j) != c.shiftIdx && !p.wishLock[i][j] && p.canDo(i, c.shiftIdx)) aCnt++
            if (aCnt == 0) return null
            var pickA = rng.nextInt(aCnt); var ai = 0
            for (i in 0 until p.S) if (p.sgrp[i] == c.groupIdx && eval.at(i, j) != c.shiftIdx && !p.wishLock[i][j] && p.canDo(i, c.shiftIdx)) { if (pickA-- == 0) { ai = i; break } }
...
internal fun findC41sFix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    return when {
        cnt > c.u -> {
            var wCnt = 0
            for (i in 0 until p.S) if (p.ssk[i] == c.groupIdx && eval.at(i, j) == c.shiftIdx && !p.wishLock[i][j]) wCnt++
            if (wCnt == 0) return null
            var pickW = rng.nextInt(wCnt); var ci = 0
            for (i in 0 until p.S) if (p.ssk[i] == c.groupIdx && eval.at(i, j) == c.shiftIdx && !p.wishLock[i][j]) { if (pickW-- == 0) { ci = i; break } }
            val allowed41 = p.allowedShiftsForStaff(ci)
            var oCnt = 0; for (ak in allowed41) if (ak != c.shiftIdx) oCnt++
            if (oCnt == 0) return null
            var pickK = rng.nextInt(oCnt); var nwK = 0
        ...
        cnt < c.l -> {
            var aCnt = 0
            for (i in 0 until p.S) if (p.ssk[i] == c.groupIdx && eval.at(i, j) != c.shiftIdx && !p.wishLock[i][j] && p.canDo(i, c.shiftIdx)) aCnt++
            if (aCnt == 0) return null
            var pickA = rng.nextInt(aCnt); var ai = 0
            for (i in 0 until p.S) if (p.ssk[i] == c.groupIdx && eval.at(i, j) != c.shiftIdx && !p.wishLock[i][j] && p.canDo(i, c.shiftIdx)) { if (pickA-- == 0) { ai = i; break } }
            intArrayOf(ai, j, c.shiftIdx)
        }
        else -> null
    }
...
internal fun findRangeHighFix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    var dayCnt = 0
    for (j in 0 until p.T) if (eval.at(rhI, j) == rhK && !p.wishLock[rhI][j]) dayCnt++
    if (dayCnt == 0) return null
    var pickJ = rng.nextInt(dayCnt); var day = 0
    for (j in 0 until p.T) if (eval.at(rhI, j) == rhK && !p.wishLock[rhI][j]) { if (pickJ-- == 0) { day = j; break } }
    ...
    for (di in 0 until p.S) {
        ...
        while (j <= p.T - D) {
            if (eval.at(i, j) == seq[0]) {
                ...
                if (miss == 1 && missL >= 0) {
                    val ml = j + missL
                    if (!p.wishLock[i][ml] && p.canDo(i, seq[missL])) return intArrayOf(i, ml, seq[missL])
                }
            }
            j++
        }
...
internal fun findAptFix(p: Problem, eval: DeltaEvaluator, rng: Random): IntArray? {
    ...
    for (i in order) {
        ...
        for (d in 0 until p.T) {
            ...
            if (!p.wishLock[i][j] && eval.at(i, j) == kOver) return intArrayOf(i, j, kUnder)
        }
    }
    return null
}
