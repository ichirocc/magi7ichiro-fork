...
object V6SanityPort {
    ...
    fun buildGuidance(state: MagiState, p: Problem = Problem(state)): List<SettingIssue> {
        ...
        for (k in 0 until p.K) {
            ...
            // [休の誤検知除外] 休(restIdx)は「1日に何人休んでよいか」という座席上限の概念を持たない
            //   （need1/need2は通常未設定=0で、これは"座席が無い"ことの表現であり"休むべきでない"ではない）。
            //   この検査は座席数が有限な勤務シフト向けの設計のため、休だけは需要上限=0でも矛盾ではない。
            //   3.76.0のC1壁判定等、既存の「休は特別扱い」方針と整合させ、休をこの検査対象から除外する
            //   （ユーザー実機報告「必要数の合計は0回の理由」＝この誤検知を指摘・確認のうえ対応）。
            if (k != p.restIdx && aptSum > seatsHi) {
