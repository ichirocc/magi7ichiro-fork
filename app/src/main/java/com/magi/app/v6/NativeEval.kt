...
object NativeEval {
    ...
    fun createHandle(p: Problem): Long {
        ...
        // [3.603.0] JNI meta 配列は Int 固定＝休が無い設定は -1 を渡す（C++ 側の「休なし」センチネルと同じ扱い）。
        val meta = intArrayOf(s, t, k, g, p.restIdx ?: -1, p.dow0, if (p.use2) 1 else 0, if (p.quantitativeRangeEval) 1 else 0)
