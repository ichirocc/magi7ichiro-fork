 ...
 *    grep で数える（教訓#31）。
 *  - 例外＝**希望どうしの衝突**: c3n(9000)→pref(8000) は HARD 件数が同じで weighted が下がるので、採否は崩した盤面を採る。
 *    盤面ごと写す経路と「希望どおりの値だけ守る」段はこの例外で実際に漏れた（2026-09-25）＝規則 A（`PolishGate.wishPinStrict`）
 *    の経路ごとの試験は `WishPinStrictTest`。
 */
