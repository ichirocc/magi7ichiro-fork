...
**最終更新**：2026-07-28（3.295.0 ブロック巡回交換に禁止連続(c3n)の事前フィルタを導入。3.294.0 のピン保存と合わせ、採用0の構造的要因（ピン破り・必須増）を両方とも0にし、残る不採用は全て soft のトレードオフ＝正直な「改善手なし」状態に到達。3.214.0〜3.294.0 の詳細は `CLAUDE.md` の各節に記録）／ **コード基準コミット**：main HEAD（この目次が古いと他が正しくても信頼が崩れるため、改修時は対象文書と本目次を必ず更新）。
