...
**最終更新**：2026-07-27（3.291.0 ブロック交換の候補生成を「希望固定日は据え置き、動かせる日だけ交換」へ緩和＝実データで候補4件→2,267件・初めて採用が発生。3.214.0〜3.290.0 の詳細は `CLAUDE.md` の各節に記録）／ **コード基準コミット**：main HEAD（この目次が古いと他が正しくても信頼が崩れるため、改修時は対象文書と本目次を必ず更新）。
