...
**最終更新**：2026-07-28（3.293.0 ブロック巡回交換の「採用0の理由」をログ化＝ピン破り／必須増(c3n)／重み悪化を isBetter の判定順で分類し悪化の主因族を併記。実データでは採用0の55〜80%が厳密ピン破り・残りは境界の禁止連続と判明。3.214.0〜3.292.0 の詳細は `CLAUDE.md` の各節に記録）／ **コード基準コミット**：main HEAD（この目次が古いと他が正しくても信頼が崩れるため、改修時は対象文書と本目次を必ず更新）。
