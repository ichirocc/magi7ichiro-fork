...
**最終更新**：2026-07-28（3.294.0 ブロック巡回交換にピン保存交換を導入＝厳密ピン(lo==hi)の回数が動かない部分集合だけを交換することで、採用0の最大要因だったピン破りを3データセットとも0へ。残る壁は境界の禁止連続(c3n)。3.214.0〜3.293.0 の詳細は `CLAUDE.md` の各節に記録）／ **コード基準コミット**：main HEAD（この目次が古いと他が正しくても信頼が崩れるため、改修時は対象文書と本目次を必ず更新）。
