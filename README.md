...
**最終更新**：2026-09-08（3.511.2＝循環交換の k=4,5 拡張（iter14 で採否）。3.511.1 停滞時の探索幅拡大は iter13 で不合格（170ペア全件無変化、既定OFF維持）。3.511.0 長期ブロック交換の候補長を動的化は iter12 で不合格（大・充足不能で必須+1、既定OFF維持）。lnsAdaptive の既定 ON は判断待ち）
