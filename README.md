...
**最終更新**：2026-08-29（3.469.0＝外部レポートの「ビット幅の境界」指摘は実コードで反証（`C3nBitScan` は
`starts>=64`/`coerceAtMost(63)` で既にガード済み）だったが、**同じ形を周辺へ当て直すと実在が2件**。
①`breakableDaysFor` だけが `C3nRowScan` 窓口（3.305.0＝呼び出し側がビット経路を分岐しないための単一入口）を
迂回し、T>64 で候補を作らず j±1 へ落としていた ②巡回交換の署名パック `ni shl (8*depth)` に幅の上限が無く、
depth=8 で mod 64 折り返し→巡回に同じ職員が2回入り「日ごとのシフト多重集合の保存」が破れる（既定 maxCycle=5 で
到達しない防御ガード＝3.369.0 と同じ立ち位置）。**自分の初版の主張1件と always-green テスト1件を教訓#30 が捕まえ、
落ちるテストへ差し替えた**（戻すと `expected:<[5,6]> but was:<[6,8]>` で該当1件だけが落ちる）。
前版は 3.468.0＝作業記録205節を`docs/history/`へ移し索引だけ残してコンテキスト固定費を 422k→202k に。）
