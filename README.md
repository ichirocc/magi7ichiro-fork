...
**最終更新**：2026-08-30（3.470.0＝CLAUDE.md 本体から版数付き作業記録を**全部**出した。3.468.0 は203節を移したが
本体にはまだ148節・4,393行（本文の83%）が残っていた。規則は「見出しに版数があるか」だけ＝機械的に検証できる形に限り、
残る21節はすべて版数を持たない恒久の事実（方針・制約の意味・重み階層・アーキテクチャ・決定記録・検証手段・バックログ）。
**5,515行/684,900B → 1,303行/181,572B（−73.6%）＝毎ターンの固定費 202k→約54k-tok**。
移動前に**決定記録 D4/D5/D6 が版数付き節の中に埋もれていた**ことを検出し、新設した恒久節「決定記録（D3〜D8・E5）」へ
D7/D8/E5 とあわせて集約（再提案しないと決めた内容が視界から消えるのを防いだ）。検証＝非空行11,434種の多重集合一致で
欠落0（初版はここで10行の欠落を検出して修正）・索引354件と本文354見出しの全単射。
前版は 3.469.0＝ビット幅の境界を周辺へ当て直し、窓口の迂回1件と署名パック幅の無保証1件を修正。）
