...
**最終更新**：2026-07-27（3.290.0 可変長ブロック交換＝11/13/17/19/23/28日の非等間隔ポートフォリオ×異グループ間交換で、15日固定・同一グループ限定では到達できなかった長期トレードを探索。3.214.0〜3.289.0 の詳細は `CLAUDE.md` の各節に記録）／ **コード基準コミット**：main HEAD（この目次が古いと他が正しくても信頼が崩れるため、改修時は対象文書と本目次を必ず更新）。
