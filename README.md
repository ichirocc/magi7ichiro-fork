...
**最終更新**：2026-09-08（3.510.3＝族別 Credit/Debt の可視化: 完了カード 2 行目「改善 …（重み N）／悪化 …（重み M）」と「後処理 収支」ログ。採否判定は不変。台帳設計の件数上限ゲートは机上評価で入れない推奨（3.510.2 補遺）、lnsAdaptive の既定 ON は判断待ち）
