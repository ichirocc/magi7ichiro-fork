...
**最終更新**：2026-09-08（3.511.1＝停滞時の探索幅拡大 `StallEscalationConfig`（iter13 で採否）。3.511.0 長期ブロック交換の候補長を動的化（iter12 で採否）。iter10/iter11 不合格（WeightDebt/ConstraintRepairInference）、lnsAdaptive の既定 ON は判断待ち）
