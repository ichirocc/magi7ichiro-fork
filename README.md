...
**関連リポジトリ**: `ichirocc/-MAGI_PC`（C# 移植＝エンジン同値、同日同期）。派生 `MAGI-Godot` は 2026-09-15 に削除（同期不要）。
...
**最終更新**：2026-09-26（手動固定（backlog #41）: セル編集シートの［固定する］で、そのセルを最適化器が書き換えない。手の編集は可・採点は不変・手動固定は希望より強い。Kotlin・C++・C# 同日）

**最終更新**：2026-09-26（希望固定セルの規則 A: 最適化器は希望固定セルを希望へ戻すか今のままにするだけ＝盤面ごと写す経路と「希望どおりの値だけ守る」段を塞いだ。あとから足した希望は従来どおり載る。240 秒の測定で合格＝既定 ON）
...
**最終更新**：2026-09-25（セル編集シートの再設計: 画面下の固定パネル・固定配置のシフトボタン・原因と相手を言う 1 行の状態・おすすめ/警告の印・希望を守っている板挟みは「他の人で補う」・即反映＋元に戻す・「必須違反 N件を順に見る」巡回＝採点・探索は不変）

**最終更新**：2026-09-25（勤務表の表示専用の印: 回数は名前の横の ▼/▲、人員は日付の下に「休▲」、期間の制約は窓ごとに印、隠れた族に点、セルシートに職員の回数・偏り、チップ「並び」＝採点・探索は不変）

**最終更新**：2026-09-25（画面層の精読 #2 の残り: UI 層 52 件の是正＝他の案は当てても残す・同じ行の重複と成り立たない値をダイアログで断る・1 操作＝1 undo・画面語彙の統一、2 回目の是正 18 件＝背景の停止と後始末・結果の旗・表示色の undo・並び以外の重複の案内、探索は不変）
...
**最終更新**：2026-09-25（既定OFF台帳の整理⑨: 否決済み `c3PairMaskEnabled` と `C3PairMaskPolish` を撤去＝既定出力不変）

**最終更新**：2026-09-25（`handleOptimize` に任意の `seed` 引数＝既定出力不変、backlog #38 D2・#39 SI-3 登録）
...
（budgetSec=20〜30s）では超過0件＝このケース規模・速いホストJVMでは踏みにくい一次データ。正式な採否
判定は複数seed・十分な予算での実行が別途必要。hosttest 823件緑。詳細は`docs/history/3.4xx.md`）
...
測定不能と判明。詳細は`docs/history/3.4xx.md`）
...
オプトイン切替は残す。詳細は`docs/history/3.4xx.md`）

**最終更新**：2026-09-17（3.590.0＝backlog#27①（fairの候補生成式）をgrillingで詰めて実装。
`AptFairPolish`のfair候補分類を`fairTarget`（生回数平均）から`fairDevOfBucket`の黒箱観測へ揃え、
`PostOptimizationParams.fairAchievementDirection`（既定OFF）でA/B切替可能に。`FairAchievementDirectionTest`
で取りこぼしの再現・修正を固定。ホストJVM 812テスト緑。tools/loopのA/Bベンチ実行中、既定ON/OFFの
判断は結果を見て別途記録。詳細は`docs/history/3.4xx.md`）
...
コード変更なし。詳細は`docs/history/3.4xx.md`）

**最終更新**：2026-09-17（3.585.0＝PR #202のCI（v6-engine-check）赤対応。3.580.0の`V6HotfixPasses.kt`
編集で既知の空振り対象行が934→951へずれたのを`weight_lint.py`が検知、行番号を更新。ホストJVM
807テスト緑。詳細は`docs/history/3.4xx.md`）
...
（生成・採否は逐次）。決定論モードの盤面ハッシュは実データ 4 件で旧新一致・時間 −17〜−40%。C# 同日同期。
...
CLAUDE.md）／3.550 色ピッカーの形を統一（文字見出しなし）／3.551 内部用語を一般画面から外す（「破壊再構築」→「組み替え」）／3.553 「計算」→「最適化」の適用漏れ回収（通知・バブル・busy 表示、3.520.0 決定は不変）／screen_spec §13 エラー・空状態・結果通知の規則を明文化（文書のみ）／3.554 人員過剰の退避研磨 `CovOReliefPolish`（基幹シフトの過剰→B4 等の単セル手を後処理の最終段で総当たり）／3.555 休0日の窓LNS `RestZeroWindowLns`（夜勤列の列挙＋人間移動＋ビーム、既定 OFF）／3.556 重み変更（c41/c42 1→9・c41s/c42s 6→10・c3m 10→6、HF77 明示指示）／
