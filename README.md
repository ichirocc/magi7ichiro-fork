...
**最終更新**：2026-09-08（3.510.5＝違反連結成分修復の一時負債予算 `ConstraintRepairInference` は iter11 で不合格（実データ全件同一、充足不能で必須 ±）＝既定 OFF 維持。3.510.4 共同 LNS の `WeightDebt` は iter10 で不合格＝既定 OFF 維持。lnsAdaptive の既定 ON は判断待ち）
