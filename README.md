...
**最終更新**：2026-09-08（3.510.2＝iter8: C3 選択日ペア交換は品質 +0.04%（実データのみ）でゲート不合格→既定 OFF 維持。次の腕「共同 LNS の短い試行→採用時だけ本予算」は iter9 で退行 0・品質 ±0・速度 平均 +10%（実データ −23%）＝既定 ON はユーザー判断待ち）
