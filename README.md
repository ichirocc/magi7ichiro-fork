...
**最終更新**：2026-09-08（3.510.5＝違反連結成分修復の一時負債予算 `ConstraintRepairInference` を opt-in 実装（iter11 で採否）。3.510.4 共同 LNS の `WeightDebt` は iter10 で不合格＝既定 OFF 維持。lnsAdaptive の既定 ON は判断待ち）
