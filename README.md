...
**最終更新**：2026-09-15（3.543.0〜3.554.0＝画面まわりの一連。3.543〜3.546 全画面 CUD 見直し＝シフト色 41 色・違反色 30 色を
...
CLAUDE.md）／3.550 色ピッカーの形を統一（文字見出しなし）／3.551 内部用語を一般画面から外す（「破壊再構築」→「組み替え」）／3.553 「計算」→「最適化」の適用漏れ回収（通知・バブル・busy 表示、3.520.0 決定は不変）／screen_spec §13 エラー・空状態・結果通知の規則を明文化（文書のみ）／3.554 人員過剰の退避研磨 `CovOReliefPolish`（基幹シフトの過剰→B4 等の単セル手を後処理の最終段で総当たり）／
