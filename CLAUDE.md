...
- **apt**（適切回数=`groupShiftApt[群][シフト]` の**群単位双方向目標**, SOFT, 重み1, L1偏差`|回数-目標|`）。
  ...
  **群目標は個人の staffRange[lo,hi] と「構造的に到達できる範囲」でクランプ**（範囲外の群目標は到達不能＝解消不能な幻のapt違反を防ぐ。
  例: Dﾃを2-2固定の職員に群目標10 → 2。休 10〜10・有 1〜1 で他シフトが上限 0 の職員は B4 ≥ 20 なので群目標 1 → 20（3.508.0、
  到達下限 = T − Σ他の置けるシフトの実効上限、到達上限 = T − Σ他シフトの実効下限。実効値は希望固定込み。個人 [lo,hi] と矛盾すれば個人優先）。
  評価は実効目標 `Problem.apt`、設定ミス診断 6b/6d/6-C は設定値 `Problem.aptRaw` を読む）。低/高(staffRange) とは別系統（LimMin/LimMax は別画面 ws5）。
