...
- **実データA/B（後処理研磨のみ・3データセット）**: アップロードされた 2026-08 実 state で
  **weighted 33179→33141・c1 54→49（−9%）**（total は +4＝weighted優先の正当な取引）。これは同セッションで
  試した独自ソルバー（行ビームDP×座標降下＋反復局所探索・35万候補評価・740回再開＝採用0）とブロック交換probe
  （部分交換版も採用0）のどちらも到達できなかった改善で、**この盤面で初めて動いた手**。
  golden_state / real_state は完全同一（採用0・退化なし・pin-regressions=0）＝中立。
- 検証: ホストJVM **全329テスト green**（新規2件=異グループ×11日ブロックで旧パスが0採用の局面を新パスが解消・
