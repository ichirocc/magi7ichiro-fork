...
- 派生 `ichirocc/MAGI-Godot`（Godot UI 版）は 2026-09-15 に削除済み＝同期・互換の義務なし（ユーザー指示、3.552.2）。
  3.552.0 で取り込んだ起動診断・`.so` 整列検査はこの repo 単体の機能として維持する。
...
- **画面文言の用語は「グループ」に統一**（「群」は使わない。制約名も「グループのレンジ／グループペア禁止／スキルグループ」、
...
## 必要時に読むもの
- `docs/architecture.md`（ファイルと役割、レイヤ）／ `docs/backlog.md`（開いている課題の一覧はこちらが正。精読 #2 は全領域完了、E5 #5 は明示 go まで保留）／
